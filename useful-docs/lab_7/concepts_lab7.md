...
<button onClick={toggleRole}>
  Switch to {role === "owner" ? "Viewer" : "Owner"}
</button>;
...
**Two types of middleware you need:**

#### `authenticate` — Validates JWT only (sends 401)
...
#### `requireOwner` — Checks role only (sends 403)
...
**Key rule:** Middleware sends only their specific codes (401 for auth, 403 for role). Everything else is the controller's job.
...
const specs = swaggerJsdoc({
  definition: {
    openapi: "3.0.0",
    info: { title: "PIT API", version: "1.0.0" },
  },
  apis: ["./routes/*.js"],
});
